...
- **ZSH プラグイン**: `brew install <plugin>` して `.zshrc` で `source`（プラグインマネージャ無し）
- **エイリアス**: `.zsh/plugins/aliases.zsh` を編集
- **自作関数**: `.zsh/plugins/foo.zsh` として追加
- **Claude Code 設定**: `home/.claude/settings.json` / `home/.claude/hooks/` を編集
- **シェル環境変数 / PATH**: `.zprofile`、対話シェルは `.zshrc` を編集
